...
if (!isset($_SESSION['admname'])) {
    ...
    foreach ($_REQUEST as $variable) {
        if (is_numeric($variable)) { //it is because, some session values are also passed with request
            ...
            if (!@executeQuery("delete from subject where subid=$variable")) {
                if (mysqli_errno ($conn) == 1451) //Children are dependent value
                    ...
                    $_GLOBALS['message'] = mysqli_errno($conn);
            }
        }
    }
    ...
    if (empty($_REQUEST['subname']) || empty($_REQUEST['subdesc'])) {
        ...
        if (!@executeQuery($query)) {
            if (mysqli_errno ($conn) == 1062) //duplicate value
...
?>
                    <table cellpadding="20" cellspacing="20" style="text-align:left;margin-left:15em" >
                        <tr>
                            <td>Subject Name</td>
                            <td><input type="text" name="subname" value="" size="16" onkeyup="isalphaWhitspace(this)" onblur="if(this.value==''){alert('Subject Name is Empty');this.focus();this.value='';}"/></td>
...
<?php                  
                    while ($r = mysqli_fetch_array($result)) {
                        ...
                        . "<td class=\"tddata\"><a title=\"Edit " . htmlspecialchars_decode($r['subdesc'], ENT_QUOTES) . "\"href=\"submng.php?edit=" . htmlspecialchars_decode($r['subname'], ENT_QUOTES) . "\"><img src=\"../images/edit.png\" height=\"30\" width=\"40\" alt=\"Edit\" /></a></td></tr>";
                    }  
?>                  
                    </table>
<?php
